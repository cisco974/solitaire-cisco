...
const getInitialState = (): SpiderGameState => {
  const baseState = {
    ...
    foundationPiles: [],
    stock: [],
    difficulty: 'medium' as SpiderDifficulty,
...
export function useSpiderGameState() {
  ...
  const updateState = useCallback((newState: Partial<SpiderGameState>) => {
    setGameState(prev => {
  ...
  const addMove = useCallback((action: SpiderGameAction) => {
  ...
  const setDifficulty = useCallback((difficulty: SpiderDifficulty) => {
    updateState({ difficulty });
  }, [updateState]);

  const setMode = useCallback((mode: SpiderMode) => {
    updateState({ mode });
  }, [updateState]);
