...
export function KlondikeGame({
  ...
  const [magicMoveTarget, setMagicMoveTarget] = useState<{
    pileIndex: number;
    card: Card;
  } | null>(null);
  const [magicMoveSource, setMagicMoveSource] = useState<{
    pileIndex: number;
    cardIndex: number;
  } | null>(null);
  ...
  const startNewGame = useCallback(() => {
    ...
    const newTableauPiles: Card[][] = Array(7)
      .fill([])
      .map(() => []);
  ...
  // Dans votre useEffect actuel à la ligne 147 environ
  useEffect(() => {
    const { isComplete, startTime } = gameState;
  ...
  const baguetteMagique = () => {
    ...
    if (possibleMoves.length > 0) {
      ...
      for (
        let i = 0;
        i < gameState.tableauPiles.length && sourcePileIndex === -1;
        i++
      ) {
      ...
      if (sourcePileIndex !== -1 && sourceCardIndex !== -1) {
        ...
        const newStock = gameState.stock.filter(
          (stockCard) =>
            !(stockCard.suit === card.suit && stockCard.rank === card.rank),
        );

        // Retirer la carte de la défausse si elle y est présente
        const newWaste = gameState.waste.filter(
          (wasteCard) =>
            !(wasteCard.suit === card.suit && wasteCard.rank === card.rank),
        );
  ...
  const handleMagicMoveComplete = () => {
    if (magicMoveTarget && magicMoveSource) {
      ...
      const { pileIndex: sourcePileIndex, cardIndex: sourceCardIndex } =
        magicMoveSource;
      ...
      newFoundationPiles[targetPileIndex] = [
        ...newFoundationPiles[targetPileIndex],
        card,
      ];
  ...
  const handleDrawCard = () => {
    ...
    if (gameState.stock.length === 0) {
      setIsRecycling(true);
      const reversedWaste = [...gameState.waste]
        .reverse()
      ...
      const cardsToDrawActual = Math.min(
        cardsToDrawCount,
        gameState.stock.length,
      );
      const newCards = gameState.stock
        .slice(-cardsToDrawActual)
  ...
  const handleDrop = (
    ...
    if (targetType === "foundation") {
      ...
      const [card] = cards;
      if (
        !isValidFoundationMove(card, gameState.foundationPiles[targetIndex])
      ) {
        playError();
        return;
      }
      ...
      if (sourceType === "tableau") {
        ...
        newFoundationPilesSource[sourceIndex] = newFoundationPilesSource[
          sourceIndex
        ].slice(0, -1);
    ...
    if (targetType === "tableau") {
      ...
      if (sourceType === "tableau") {
        ...
        const newFoundationPiles = [...gameState.foundationPiles];
        newFoundationPiles[sourceIndex] = newFoundationPiles[sourceIndex].slice(
          0,
          -1,
        );
  ...
  const isCardHighlighted = (
    ...
    return (
      highlightedMove.from.type === sourceType &&
      ...
      <div className="relative w-20 h-32">
        <AnimatePresence mode="popLayout">
          {visibleCards.map((card, index) => {
            ...
            const wasteIndex =
              gameState.waste.length - visibleCards.length + index;
            const isHighlighted =
              highlightedMove?.from.type === "waste" &&
            ...
            return (
              ...
              >
                <CardComponent
                  ...
                  draggable={isTopCard}
                  isDragging={
                    draggedCards?.sourceType === "waste" &&
                    draggedCards.sourceIndex === 0 &&
                    wasteIndex >= draggedCards.cardIndex
                  }
                  onDragStart={(e) =>
                    handleDragStart(e, [card], "waste", 0, wasteIndex)
                  }
          ...
          {gameState.tableauPiles.map((pile, pileIndex) => (
            ...
            >
              {pile.map((card, cardIndex) => {
                ...
                const isMagicMoveSource =
                  magicMoveSource?.pileIndex === pileIndex &&
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      zIndex: cardIndex,
                      opacity:
                        (draggedCards?.sourceType === "tableau" &&
